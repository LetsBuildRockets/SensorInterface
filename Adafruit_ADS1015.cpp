...
/*!
    ...
    @author   K. Townsend (Adafruit Industries) --> Eric Sims overwrote this for arm junk :)
...
uint16_t Adafruit_ADS1015::readRegister(uint8_t i2cAddress, uint8_t reg) {
  char cmd[1];
  cmd[0] = ADS1015_REG_POINTER_CONVERT;
  i2c.write((i2cAddress << 1), cmd, 1);
  
  char rtn[2];
  i2c.read((i2cAddress << 1), rtn, 2);
  return ((rtn[0] << 8) | rtn[1]);
...
/**************************************************************************/
/*!
    @brief  Gets a single-ended ADC reading from all channels
*/
/**************************************************************************/

bool Adafruit_ADS1015::readADC_SingleEndedAll(int16_t &a, int16_t &b, int16_t &c, int16_t &d) {
    a = readADC_SingleEnded(0);
    b = readADC_SingleEnded(1);
    c = readADC_SingleEnded(2);
    d = readADC_SingleEnded(3);
    return true;
}
